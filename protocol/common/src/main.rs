...
use common::types::{BuildTransactionError, SignTransactionError};
...
fn get_p2pkh_address(private_key_wif: String, network: NetworkCandid) -> String {
    ...
    common::get_p2pkh_address(&private_key, network).to_string()
}
...
fn build_transaction(
    ...
    let outpoint_to_index: HashMap<OutPoint, usize> = utxos
        ...
        .collect();
    let tx = common::build_transaction(
        utxos,
...
fn sign_transaction(
    ...
    Ok(common::sign_transaction(tx, private_key, source_address).serialize())
}
