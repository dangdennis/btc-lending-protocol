...
if [ "$(uname)" == "Darwin" ]; then
  ...
  AR="/usr/local/opt/llvm/bin/llvm-ar" CC="/usr/local/opt/llvm/bin/clang" cargo build --bin common --target $TARGET --release
else
  cargo build --bin common --target $TARGET --release
fi
