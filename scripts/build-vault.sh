...
if [ "$(uname)" == "Darwin" ]; then
  ...
  AR="/usr/local/opt/llvm/bin/llvm-ar" CC="/usr/local/opt/llvm/bin/clang" cargo build --bin vault --target $TARGET --release
else
  cargo build --bin vault --target $TARGET --release
fi
