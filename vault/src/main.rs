...
thread_local! {
    // Set to dummy values. To be initialized.
    static BTC_PRIVATE_KEY: RefCell<PrivateKey> =
        RefCell::new(PrivateKey::from_wif("").unwrap());
...
struct InitPayload {
    ...
    bitcoin_private_key_wif: String,
}
...
fn init(payload: InitPayload) {
    ...
    BTC_PRIVATE_KEY.with(|pk| {
        pk.replace(PrivateKey::from_wif(&payload.bitcoin_private_key_wif[..]).unwrap());
    })
}
